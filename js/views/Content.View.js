define(["jquery", "underscore", "backbone", "text!app/templates/Content.Template.html"], function($, _, Backbone, ContentTemplate)
{
	var ContentView = Backbone.View.extend({
		...
		render : function()
		{
			...
			/*
			 var childrenUnits = ["children", "metadata", "units"];
			 this.$el.html(_.template(ContentTemplate, this.obj));

			 var that = this;
			 _.each(childrenUnits, function(childUnit)
			 {
			 if (that.obj[childUnit]) {
			 _.each(that.obj[childUnit], function(child)
			 {
			 console.log('rendering children '+childUnit);
			 var childView = new ContentView({
			 obj : child,
			 parent : that,
			 app : that.app
